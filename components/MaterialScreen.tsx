...
const MaterialScreen: React.FC<{ orders?: any[], isAdmin: boolean }> = ({ orders = [], isAdmin }) => {
  ...
  const saveDraft = (newData: any) => {
    try {
      const existing = JSON.parse(localStorage.getItem(DRAFT_KEY) || "{}");
      localStorage.setItem(DRAFT_KEY, JSON.stringify({ ...existing, ...newData }));
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const payload = {
        client: client.toUpperCase().trim(), cascos, puertas, tiradores,
  ...
  const handleSearch = (val: string) => {
    ...
    } else if (scrollContainerRef.current && listHeaderRef.current) {
      scrollContainerRef.current.scrollTo({ top: listHeaderRef.current.offsetTop - 120, behavior: 'smooth' });
    }
  };
    ...
    <div className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden relative">
      ...
      <div className="bg-slate-950/95 backdrop-blur-md p-3 border-b border-slate-900 sticky top-[60px] z-20">
        <div className="max-w-xl mx-auto px-2 flex gap-2">
          ...
          <button onClick={() => handleSearch("")} className="bg-red-900/20 px-4 rounded-xl text-red-500 font-black text-[10px] uppercase active:scale-90 transition-all">LIMPIAR</button>
        </div>
      </div>
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto no-scrollbar">
        {isAdmin && (
          <div className="max-w-xl mx-auto px-5 py-6 space-y-4">
             <input value={client} onChange={e => { setClient(e.target.value); saveDraft({ client: e.target.value }); }} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none focus:border-blue-500 transition-all" placeholder="CLIENTE" />
             ...
             <div className="flex gap-2">
               <div className="flex-1 relative bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-center gap-3 active:bg-slate-800 transition-all">
                 <Camera size={20} /> <span className="text-[10px] font-black uppercase">Subir Foto</span>
                 <input type="file" onChange={(e) => { if(e.target.files?.[0]) { setImageFile(e.target.files[0]); setImagePreview(URL.createObjectURL(e.target.files[0])); } }} accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" />
               </div>
               {imagePreview && <img src={imagePreview} className="h-14 w-14 rounded-xl object-cover border border-slate-800 shadow-lg" />}
             </div>

             <select value={estado} onChange={e => { setEstado(e.target.value); saveDraft({ estado: e.target.value }); }} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none transition-all">
             ...
             <div className="grid grid-cols-3 gap-2">
               {['CASCOS', 'PUERTAS', 'TIRADORES'].map(f => (
                 <label key={f} className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-2 cursor-pointer active:bg-slate-800 transition-all">
                   ...
                   <input type="checkbox" checked={f==='CASCOS'?cascos:f==='PUERTAS'?puertas:tiradores} onChange={e => {
                     const checked = e.target.checked;
                     if(f==='CASCOS') { setCascos(checked); saveDraft({ cascos: checked }); }
                     else if(f==='PUERTAS') { setPuertas(checked); saveDraft({ puertas: checked }); }
                     else { setTiradores(checked); saveDraft({ tiradores: checked }); }
             ...
             <div className="grid grid-cols-2 gap-2">
               <input value={rack} onChange={e => { setRack(e.target.value); saveDraft({ rack: e.target.value }); }} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none transition-all" placeholder="RACK" />
