    ...
    <div className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden relative">
      {fullscreenImg && (
        <div className="fixed inset-0 z-[110] bg-black flex items-center justify-center p-6">
          ...
          <TransformWrapper initialScale={1} centerOnInit={true}><TransformComponent wrapperStyle={{ width: "100vw", height: "100vh" }}><img src={fullscreenImg} className="max-w-full max-h-screen object-contain rounded-xl shadow-2xl" /></TransformComponent></TransformWrapper>
        </div>
      )}
      <div className="sticky top-0 z-30 bg-blue-600 shadow-xl"><div className="max-w-xl mx-auto px-5 py-4 flex items-center gap-2"><button onClick={() => navigate("/menu")} className="active:scale-90 transition-transform"><ChevronLeft size={28} /></button><h1 className="text-lg font-black uppercase tracking-widest leading-none text-white">Material</h1></div></div>
      <div className="bg-slate-950/95 backdrop-blur-md p-3 border-b border-slate-900 sticky top-[60px] z-20"><div className="max-w-xl mx-auto px-2 flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={16} /><input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-slate-900 border border-slate-800 p-3 pl-10 rounded-xl text-white font-black uppercase text-xs outline-none focus:border-blue-500 transition-all" placeholder="BUSCAR CLIENTE..." /></div><button onClick={() => setSearch("")} className="bg-red-900/20 px-4 rounded-xl text-red-500 font-black text-[10px] uppercase active:scale-90 transition-all">LIMPIAR</button></div></div>
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto no-scrollbar">
        {isAdmin && (
          <div className="max-w-xl mx-auto px-5 py-6 space-y-4">
             <input value={client} onChange={e => setClient(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none focus:border-blue-500 transition-all" placeholder="CLIENTE" />
             <div className="flex gap-2"><div className="flex-1 relative bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-center gap-3 active:bg-slate-800 transition-all"><Camera size={20} /> <span className="text-[10px] font-black uppercase">Subir Foto</span><input type="file" onChange={e => { if(e.target.files?.[0]) { setImageFile(e.target.files[0]); setImagePreview(URL.createObjectURL(e.target.files[0])); } }} accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" /></div>{imagePreview && <img src={imagePreview} className="h-14 w-14 rounded-xl object-cover border border-slate-800 shadow-lg" />}</div>
             <select value={estado} onChange={e => setEstado(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none transition-all"><option value="">QUEDARSE EN MATERIAL</option><option value="ARMANDOSE">A ARMÁNDOSE</option><option value="TERMINADA">A TERMINADA</option><option value="PROGRAMADA">A ENTREGAS</option><option value="TERMINACIONES">A TERMINACIONES</option></select>
             <div className="grid grid-cols-3 gap-2">{['CASCOS', 'PUERTAS', 'TIRADORES'].map(f => (<label key={f} className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex flex-col items-center gap-2 cursor-pointer active:bg-slate-800 transition-all"><span className="text-[10px] font-black uppercase">{f}</span><input type="checkbox" checked={f==='CASCOS'?cascos:f==='PUERTAS'?puertas:tiradores} onChange={e => { const c = e.target.checked; if(f==='CASCOS') setCascos(c); else if(f==='PUERTAS') setPuertas(c); else setTiradores(c); }} className="h-4 w-4 accent-blue-500" /></label>))}</div>
             <div className="grid grid-cols-2 gap-2"><input value={rack} onChange={e => setRack(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none transition-all" placeholder="RACK" /><input value={nivel} onChange={e => setNivel(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white font-black uppercase outline-none transition-all" placeholder="NIVEL" /></div>
        ...
        <div className="max-w-xl mx-auto px-5 py-6 space-y-3 pb-40">
            {filtered.map(r => (
              <div key={r.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-3 flex items-center gap-4 min-h-[96px] shadow-sm">
                <div className="shrink-0" onClick={() => r.image_url && setFullscreenImg(r.image_url)}>{r.image_url ? <img src={r.image_url} className="h-16 w-16 rounded-xl object-cover border border-slate-800 cursor-pointer" /> : <div className="h-16 w-16 rounded-xl bg-slate-800 flex items-center justify-center text-white border border-slate-800"><ImageIcon size={20}/></div>}</div>
                <div className="flex-1 min-w-0"><h3 className="text-sm font-black uppercase truncate text-white">{r.client}</h3>{r.ubicado && <p className="text-[10px] text-slate-500 uppercase mt-1 truncate"><MapPin size={10} className="inline mr-1" />{r.ubicado}</p>}<div className="flex gap-1.5 mt-2"><span className={getBadgeClass(r.cascos)}>CA</span><span className={getBadgeClass(r.puertas)}>PT</span><span className={getBadgeClass(r.tiradores)}>TR</span></div></div>
                {isAdmin && (<div className="flex flex-col gap-2 shrink-0"><button onClick={() => { setEditingId(r.id); setClient(r.client); const parts = r.ubicado?.split(' - ') || ["",""]; setRack(parts[0]); setNivel(parts[1]); setCascos(r.cascos); setPuertas(r.puertas); setTiradores(r.tiradores); if(scrollContainerRef.current) scrollContainerRef.current.scrollTo({top: 0, behavior: 'smooth'}); }} className="p-2 bg-blue-500/10 rounded-lg text-white border border-blue-500/20 active:scale-90 transition-all"><Edit3 size={16}/></button><button onClick={async () => { if(window.confirm("¿BORRAR?")) { await supabase.from('unico_materials').delete().eq('id', r.id); fetchMaterials(); } }} className="p-2 bg-red-500/10 rounded-lg text-red-500 border border-red-500/20 active:scale-90 transition-all"><Trash2 size={16}/></button></div>)}
              </div>
            ))}
        </div>
      </div>
