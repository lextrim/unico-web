...
const InformesScreen: React.FC<{ isAdmin: boolean, isAPK: boolean }> = ({ isAdmin, isAPK }) => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const ordsFormatted = (orders || []).map(o => ({
        id: o.id,
        ...o.payload,
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        head: [["FECHA", "CLIENTE", "ESTADO", "UBICACIÓN", "CA", "PU", "TI", "NOTAS"]],
        body: tableRows,
        startY: 28,
        theme: 'grid',
        styles: { fontSize: 8 }
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden">
      <div className="bg-slate-900 shadow-xl border-b border-slate-800 sticky top-0 z-30">
        <div className="max-w-xl mx-auto px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/menu")} className="p-2"><ChevronLeft size={28} /></button>
            <h1 className="text-lg font-black uppercase">Informes</h1>
          </div>
          <button onClick={exportToPDF} disabled={exporting} className="bg-red-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
      ...
      <div className="bg-slate-950/95 p-4 border-b border-slate-900">
        <div className="max-w-xl mx-auto space-y-4">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-slate-900 border border-slate-800 p-3 pl-10 rounded-xl text-xs font-black uppercase text-white outline-none" placeholder="BUSCAR..." />
          </div>
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            {['TODOS', 'MATERIALES', 'ARMANDOSE', 'TERMINADA', 'PROGRAMADA', 'TERMINACIONES'].map(st => (
              <button key={st} onClick={() => setStatusFilter(st)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase whitespace-nowrap border ${statusFilter === st ? 'bg-white text-black border-white' : 'bg-slate-900 text-slate-400 border-slate-800'}`}>
                {st === 'PROGRAMADA' ? 'ENTREGAS' : st}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        <div className="max-w-xl mx-auto p-5 space-y-4 pb-24">
          {loading ? (
            <div className="text-center py-10 font-black text-slate-500 uppercase">Cargando...</div>
          ) : filteredRecords.map(r => (
            <div key={r.id} className="bg-slate-900 border border-slate-800 p-4 rounded-3xl flex flex-col gap-3 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-sm font-black uppercase text-white">{r.client}</h3>
                  <div className="flex items-center gap-3 mt-1.5 text-[10px] text-slate-500 font-bold uppercase">
                    <span className="flex items-center gap-1"><Calendar size={12}/> {r.date.toLocaleDateString()}</span>
                    {r.ubicado && <span className="flex items-center gap-1"><MapPin size={12}/> {r.ubicado}</span>}
                  </div>
                </div>
                <span className="text-[9px] font-black uppercase px-2 py-1 bg-slate-800 rounded-lg border border-slate-700 text-slate-400">{r.status}</span>
              </div>
            </div>
          ))}
        </div>
